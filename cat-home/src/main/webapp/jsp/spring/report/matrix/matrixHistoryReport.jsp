...
<c:set var="sortQueryPrefix" value="op=history&date=${date}&domain=${domain}&ip=${ipAddress}&reportType=${reportType}${customDate}" />
<!doctype html>
<html lang="en">
<head>
...
<body class="no-skin">
							...
							<tr>
								...
								<td>
									<div class="nav-search nav" id="nav-search">
										<span class="text-danger switch">【<a class="switch" href="${contextPath}/mvc/r/matrix?domain=${domain}&ip=${ipAddress}"><span class="text-danger">切到小时模式</span></a>】</span>
										<c:forEach var="nav" items="${historyNavs}">
											&nbsp;[ <a href="${contextPath}/mvc/r/matrix?op=history&domain=${domain}&ip=${ipAddress}&date=${date}&reportType=${nav.title}" class="${nav.title eq reportType ? 'current' : ''}">${nav.title}</a> ]
										</c:forEach>
										&nbsp;[ <a href="${contextPath}/mvc/r/matrix?op=history&domain=${domain}&ip=${ipAddress}&date=${date}&reportType=${reportType}&step=-1">${currentNav.last}</a> ]
										&nbsp;[ <a href="${contextPath}/mvc/r/matrix?op=history&domain=${domain}&ip=${ipAddress}&date=${date}&reportType=${reportType}&step=1">${currentNav.next}</a> ]
										&nbsp;[ <a href="${contextPath}/mvc/r/matrix?op=history&domain=${domain}&ip=${ipAddress}&reportType=${reportType}">now</a> ]&nbsp;
									</div>
								</td>
							</tr>
						</table>
									...
									<tr>
										...
										<td><div class="domain">
											<c:forEach var="itemDomain" items="${line.value.lineDomains}">
												&nbsp;<a class="domainItem" href="${contextPath}/mvc/r/matrix?op=history&domain=${itemDomain}&date=${date}&reportType=${reportType}${customDate}">[&nbsp;<c:out value="${itemDomain}" />&nbsp;]</a>&nbsp;
											</c:forEach>
										</div></td>
									</tr>
								</c:forEach>
					...
					<div class="row-fluid">
						<table class="table table-hover table-bordered table-striped table-condensed">
							<tr>
								...
								<th class="left" width="20%" rowspan="2"><a href="${contextPath}/mvc/r/matrix?${sortQueryPrefix}&sort=Name">Name</a></th>
							...
							<c:forEach var="item" items="${matrix.matrixs}">
								<tr class="right">
									...
									<td><a href="${contextPath}/mvc/r/m/${item.url}?domain=${domain}">L</a></td>
									<td>${item.callMin}</td>
									<td>${item.callMax}</td>
	...
	<script type="text/javascript">
		...
		function buildHref(domain) {
			return '<a href="${contextPath}/mvc/r/matrix?op=history&domain=' + domain + '&date=${date}&reportType=${reportType}${customDate}">&nbsp;[&nbsp;' + domain + '&nbsp;]&nbsp;</a>';
		}
		$(document).ready(function() {
			...
			$("#search_go").bind("click", function() {
				window.location.href = '${contextPath}/mvc/r/matrix?op=history&domain=' + $("#search").val() + '&date=${date}&reportType=${reportType}${customDate}';
			});
