...
<body class="no-skin">
							...
							<tr>
								...
								<td>
									<div class="nav-search nav" id="nav-search">
										<span class="text-danger switch">【<a class="switch" href="${contextPath}/mvc/r/cache?op=history&domain=${domain}&ip=${ipAddress}&type=${encodedType}"><span class="text-danger">切到历史模式</span></a>】</span>
										<c:forEach var="nav" items="${navs}">
											&nbsp;[ <a href="${contextPath}/mvc/r/cache?date=${date}&ip=${ipAddress}&step=${nav.hours}&${navPrefix}">${nav.title}</a> ]
										</c:forEach>
										&nbsp;[ <a href="${contextPath}/mvc/r/cache?${navPrefix}">now</a> ]&nbsp;
									</div>
								</td>
							</tr>
						</table>
	...
	<script type="text/javascript">
		...
		function buildHref(domain) {
			return '<a href="${contextPath}/mvc/r/cache?op=view&domain=' + encodeURIComponent(domain) + '&date=${date}&reportType=${reportType}">&nbsp;[&nbsp;' + domain + '&nbsp;]&nbsp;</a>';
		}
		$(document).ready(function() {
			...
			$("#search_go").bind("click", function() {
				window.location.href = '${contextPath}/mvc/r/cache?op=view&domain=' + encodeURIComponent($("#search").val()) + '&date=${date}&reportType=${reportType}';
			});
