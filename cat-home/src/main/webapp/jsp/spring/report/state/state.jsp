...
<c:choose>
	<c:when test="${historyMode}">
		<c:set var="listQueryPrefix" value="op=history&show=${show}&domain=${domain}&date=${date}&reportType=${reportType}${customDate}" />
		<c:set var="graphQueryPrefix" value="op=historyGraph&ip=${ipAddress}&date=${date}&reportType=${reportType}${customDate}" />
		<c:set var="sortQueryPrefix" value="op=history&domain=${domain}&ip=${ipAddress}&date=${date}&reportType=${reportType}${customDate}&show=true" />
	</c:when>
	<c:otherwise>
		<c:set var="listQueryPrefix" value="show=${show}&domain=${domain}&date=${date}" />
		<c:set var="graphQueryPrefix" value="op=graph&ip=${ipAddress}&date=${date}" />
		<c:set var="sortQueryPrefix" value="domain=${domain}&ip=${ipAddress}&date=${date}&show=true" />
	</c:otherwise>
</c:choose>
<!doctype html>
<html lang="en">
			...
			<div style="padding-top:2px;padding-left:2px;padding-right:8px;">
				<div class="report">
							...
							<tr>
								...
								<td>
									<div class="nav-search nav" id="nav-search">
										<c:choose>
											<c:when test="${historyMode}">
												<span class="text-danger switch">[ <a class="switch" href="${contextPath}/mvc/r/state?domain=${domain}&ip=${ipAddress}"><span class="text-danger">切到小时模式</span></a> ]</span>
												<c:forEach var="nav" items="${historyNavs}">
													&nbsp;[ <a href="${contextPath}/mvc/r/state?op=history&domain=${domain}&ip=${ipAddress}&date=${date}&reportType=${nav.title}&show=${show}" class="${nav.title eq reportType ? 'current' : ''}">${nav.title}</a> ]
												</c:forEach>
												&nbsp;[ <a href="${contextPath}/mvc/r/state?op=history&domain=${domain}&ip=${ipAddress}&date=${date}&reportType=${reportType}&step=-1&show=${show}">${currentNav.last}</a> ]
												&nbsp;[ <a href="${contextPath}/mvc/r/state?op=history&domain=${domain}&ip=${ipAddress}&date=${date}&reportType=${reportType}&step=1&show=${show}">${currentNav.next}</a> ]
												&nbsp;[ <a href="${contextPath}/mvc/r/state?op=history&domain=${domain}&ip=${ipAddress}&reportType=${reportType}&show=${show}">now</a> ]&nbsp;
											</c:when>
											<c:otherwise>
												<span class="text-danger switch">[ <a class="switch" href="${contextPath}/mvc/r/state?op=history&domain=${domain}&ip=${ipAddress}&show=${show}"><span class="text-danger">切到历史模式</span></a> ]</span>
					...
					<table class="machines">
						<tr class="left">
							<th>&nbsp;[&nbsp;
								<a href="${contextPath}/mvc/r/state?${listQueryPrefix}" class="${ipAddress eq 'All' ? 'current' : ''}">All</a>
								&nbsp;]&nbsp;
								<c:forEach var="ip" items="${ips}">
									&nbsp;[&nbsp;<a href="${contextPath}/mvc/r/state?${listQueryPrefix}&ip=${ip}" class="${ipAddress eq ip ? 'current' : ''}"><c:out value="${ip}" /></a>&nbsp;]&nbsp;
								</c:forEach>
							</th>
						</tr>
					</table>
					...
					<table class="table table-hover table-striped table-condensed" width="100%">
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=total" data-status="total" class="state_graph_link">[:: show ::]</a></td>
							<td>处理消息总量</td>
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=totalLoss" data-status="totalLoss" class="state_graph_link">[:: show ::]</a></td>
							<td>丢失消息总量</td>
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=avgTps" data-status="avgTps" class="state_graph_link">[:: show ::]</a></td>
							<td>每分钟平均处理数</td>
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=maxTps" data-status="maxTps" class="state_graph_link">[:: show ::]</a></td>
							<td>单台机器每分钟最大处理数</td>
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=dump" data-status="dump" class="state_graph_link">[:: show ::]</a></td>
							<td>压缩成功消息数量</td>
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=dumpLoss" data-status="dumpLoss" class="state_graph_link">[:: show ::]</a></td>
							<td>来不及压缩丢失消息数量</td>
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=pigeonTimeError" data-status="pigeonTimeError" class="state_graph_link">[:: show ::]</a></td>
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=networkTimeError" data-status="networkTimeError" class="state_graph_link">[:: show ::]</a></td>
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=blockTotal" data-status="blockTotal" class="state_graph_link">[:: show ::]</a></td>
							<td>存储消息块数量</td>
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=blockLoss" data-status="blockLoss" class="state_graph_link">[:: show ::]</a></td>
							<td>存储消息块丢失数量</td>
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=blockTime" data-status="blockTime" class="state_graph_link">[:: show ::]</a></td>
							<td>存储消息块花费时间(分钟)</td>
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=size" data-status="size" class="state_graph_link">[:: show ::]</a></td>
							<td>压缩前消息大小(GB)</td>
						...
						<tr>
							<td><a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=delayAvg" data-status="delayAvg" class="state_graph_link">[:: show ::]</a></td>
							<td>系统处理延迟(ms)</td>
					...
					<c:if test="${show}">
						<table class="table table-hover table-striped table-condensed" width="100%">
							<tr>
								<td width="10%"><a href="${contextPath}/mvc/r/state?${sortQueryPrefix}&sort=domain">处理项目列表</a></td>
								<td width="10%" class="right"><a href="${contextPath}/mvc/r/state?${sortQueryPrefix}&sort=total">处理消息总量</a></td>
								<td width="10%" class="right"><a href="${contextPath}/mvc/r/state?${sortQueryPrefix}&sort=loss">丢失消息总量</a></td>
								<td width="10%" class="right"><a href="${contextPath}/mvc/r/state?${sortQueryPrefix}&sort=size">压缩前消息大小(GB)</a></td>
								<td width="15%" class="right"><a href="${contextPath}/mvc/r/state?${sortQueryPrefix}&sort=avg">平均消息大小(KB)</a></td>
								<td width="5%" class="right"><a href="${contextPath}/mvc/r/state?${sortQueryPrefix}&sort=machine">机器总数</a></td>
							...
							<c:forEach var="item" items="${state.processDomains}" varStatus="status">
								<tr>
									...
									<td class="right"><fmt:formatNumber value="${item.total}" pattern="#,###,###,###,##0.#" /><br>
										<c:if test="${!historyMode}">
											<a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=${item.name}:total" data-status="${item.name}:total" class="state_graph_link">[:: show ::]</a>
										</c:if></td>
									<td class="right"><fmt:formatNumber value="${item.totalLoss}" pattern="#,###,###,###,##0.#" /><br>
										<c:if test="${!historyMode}">
											<a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=${item.name}:totalLoss" data-status="${item.name}:totalLoss" class="state_graph_link">[:: show ::]</a>
										</c:if></td>
									<td class="right"><fmt:formatNumber value="${item.size / 1024 / 1024 / 1024}" pattern="#,###,##0.000" /><br>
										<c:if test="${!historyMode}">
											<a href="${contextPath}/mvc/r/state?${graphQueryPrefix}&key=${item.name}:size" data-status="${item.name}:size" class="state_graph_link">[:: show ::]</a>
										</c:if></td>
