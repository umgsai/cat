	...
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div style="padding-top:2px;padding-right:8px;">
				<form name="topologyGraphEdgeConfigAddSumbit" id="form" method="post" action="${contextPath}/mvc/s/config?op=topologyGraphEdgeConfigAddSumbit">
					...
					<table class="table table-striped table-condensed">
						<tr>
							...
							<td>
								<select id="type" name="edgeConfig.type">
									<option value="PigeonCall" ${edgeConfig.type eq 'PigeonCall' || type eq 'PigeonCall' ? 'selected' : ''}>PigeonCall</option>
									<option value="Database" ${edgeConfig.type eq 'Database' || type eq 'Database' ? 'selected' : ''}>Database</option>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td>
								<c:choose>
									<c:when test="${empty edgeConfig.from}">
										<input id="from" name="edgeConfig.from" value="${from}" required />
									</c:when>
									<c:otherwise>
										<input id="from" name="edgeConfig.from" value="${edgeConfig.from}" required />
									</c:otherwise>
								</c:choose>
							</td>
						</tr>
						<tr>
							...
							<td>
								<c:choose>
									<c:when test="${empty edgeConfig.to}">
										<input id="to" name="edgeConfig.to" value="${to}" required />
									</c:when>
									<c:otherwise>
										<input id="to" name="edgeConfig.to" value="${edgeConfig.to}" required />
									</c:otherwise>
								</c:choose>
							</td>
						</tr>
						<tr>
							...
							<td><input id="minCountThreshold" name="edgeConfig.minCountThreshold" value="${edgeConfig.minCountThreshold}" required /></td>
						</tr>
						<tr>
							<td style="text-align:right" class="text-success">一分钟异常数warning阈值</td>
							<td><input id="warningThreshold" name="edgeConfig.warningThreshold" value="${edgeConfig.warningThreshold}" required /></td>
						</tr>
						<tr>
							<td style="text-align:right" class="text-success">一分钟内调用异常数error阈值</td>
							<td><input id="errorThreshold" name="edgeConfig.errorThreshold" value="${edgeConfig.errorThreshold}" required /></td>
						</tr>
						<tr>
							<td style="text-align:right" class="text-success">调用响应时间warning阈值</td>
							<td><input id="warningResponseTime" name="edgeConfig.warningResponseTime" value="${edgeConfig.warningResponseTime}" required /></td>
						</tr>
						<tr>
							<td style="text-align:right" class="text-success">调用响应时间error阈值</td>
							<td><input id="errorResponseTime" name="edgeConfig.errorResponseTime" value="${edgeConfig.errorResponseTime}" required /></td>
						</tr>
						<tr>
							<td>&nbsp;</td>
							<td><input class="btn btn-primary btn-sm" id="addOrUpdateEdgeSubmit" type="submit" name="submit" value="提交" /></td>
						</tr>
					</table>
				</form>
			</div>
