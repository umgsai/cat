...
<head>
	...
	<script src="${contextPath}/js/heartbeatHistory.js"></script>
	<style>
...
<body class="no-skin">
			...
			<div style="padding-top:2px;padding-left:2px;padding-right:8px;">
				<div class="report">
							...
							<tr>
								...
								<td>
									<div class="nav-search nav" id="nav-search">
										<span class="text-danger switch">[ <a class="switch" href="${contextPath}/mvc/r/h?domain=${domain}&ip=${ipAddress}"><span class="text-danger">切到小时模式</span></a> ]</span>
										<c:forEach var="nav" items="${historyNavs}">
											&nbsp;[ <a href="${contextPath}/mvc/r/h?op=history&domain=${domain}&ip=${ipAddress}&date=${date}&reportType=${nav.title}" class="${nav.title eq reportType ? 'current' : ''}">${nav.title}</a> ]
										</c:forEach>
										&nbsp;[ <a href="${contextPath}/mvc/r/h?op=history&domain=${domain}&ip=${ipAddress}&date=${date}&reportType=${reportType}&step=-1">${currentNav.last}</a> ]
										&nbsp;[ <a href="${contextPath}/mvc/r/h?op=history&domain=${domain}&ip=${ipAddress}&date=${date}&reportType=${reportType}&step=1">${currentNav.next}</a> ]
										&nbsp;[ <a href="${contextPath}/mvc/r/h?op=history&domain=${domain}&ip=${ipAddress}&reportType=${reportType}">now</a> ]&nbsp;
									</div>
								</td>
							</tr>
						</table>
									...
									<tr>
										...
										<td><div class="domain">
											<c:forEach var="itemDomain" items="${line.value.lineDomains}">
												&nbsp;<a class="domainItem" href="${contextPath}/mvc/r/h?op=history&domain=${itemDomain}&date=${date}&reportType=${reportType}">[&nbsp;<c:out value="${itemDomain}" />&nbsp;]</a>&nbsp;
											</c:forEach>
										</div></td>
									</tr>
								</c:forEach>
						...
						<tr class="left">
							<th>
								<c:forEach var="ip" items="${ips}">
									&nbsp;[&nbsp;<a href="${contextPath}/mvc/r/h?op=history&domain=${domain}&date=${date}&ip=${ip}&reportType=${reportType}" class="${realIp eq ip ? 'current' : ''}"><c:out value="${ip}" /></a>&nbsp;]&nbsp;
								</c:forEach>
							</th>
						</tr>
					</table>
					...
					<table style="width:100%">
						<c:forEach var="extensionGroup" items="${extensionGroups}">
							<c:url var="extensionHref" value="/mvc/r/h">
								<c:param name="op" value="historyPart" />
								<c:param name="domain" value="${domain}" />
								<c:param name="date" value="${date}" />
								<c:param name="ip" value="${realIp}" />
								<c:param name="reportType" value="${reportType}" />
								<c:param name="type" value="extension" />
								<c:param name="extensionType" value="${extensionGroup}" />
							</c:url>
							<tr>
								<th colspan="3" style="text-align:left"><h5><a data-status="${extensionGroup}" class="heartbeat_graph_link" href="${extensionHref}"><c:out value="${extensionGroup}" /></a></h5></th>
							</tr>
							<tr>
								<td colspan="3"><iframe id="${extensionGroup}" style="display:none;" width="100%" height="400"></iframe></td>
							</tr>
						</c:forEach>
					</table>
	...
	<script type="text/javascript">
		...
		function buildHref(domain) {
			return '<a href="${contextPath}/mvc/r/h?op=history&domain=' + encodeURIComponent(domain) + '&date=${date}&reportType=${reportType}">&nbsp;[&nbsp;' + domain + '&nbsp;]&nbsp;</a>';
		}
		$(document).ready(function() {
			...
			$("#search_go").bind("click", function() {
				window.location.href = '${contextPath}/mvc/r/h?op=history&domain=' + encodeURIComponent($("#search").val()) + '&date=${date}&reportType=${reportType}';
			});
