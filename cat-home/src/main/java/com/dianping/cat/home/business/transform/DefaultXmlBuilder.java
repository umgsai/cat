...
public class DefaultXmlBuilder implements IVisitor {
   ...
   public void visitBusinessItem(BusinessItem businessItem) {
      startTag(ENTITY_BUSINESS_ITEM, true, null, ATTR_DOMAIN, businessItem.getDomain(), ATTR_ITEM_ID, businessItem.getItemId());
   }

   @Override
   public void visitBusinessTagConfig(BusinessTagConfig businessTagConfig) {
      startTag(ENTITY_BUSINESS_TAG_CONFIG, null);

      if (!businessTagConfig.getTags().isEmpty()) {
         for (Tag tag : businessTagConfig.getTags().values()) {
            tag.accept(m_visitor);
         }
      }

      endTag(ENTITY_BUSINESS_TAG_CONFIG);
   }

   @Override
   public void visitTag(Tag tag) {
      startTag(ENTITY_TAG, null, ATTR_ID, tag.getId());

      if (!tag.getBusinessItems().isEmpty()) {
