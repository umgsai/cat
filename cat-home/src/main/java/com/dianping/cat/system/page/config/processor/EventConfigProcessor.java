...
@Component("eventConfigProcessor")
public class EventConfigProcessor extends BaseProcesser {
	...
	public void process(Action action, Payload payload, Model model) {
		switch (action) {
		case EVENT_RULE:
			Map<String, Rule> ruleMap = eventRuleConfigManager.getMonitorRules().getRules();
			rulesAvailableBuild(ruleMap);
			model.setRules(ruleMap.values());
			break;
		case EVENT_RULE_ADD_OR_UPDATE:
			generateRuleConfigContent(payload.getRuleId(), eventRuleConfigManager, model);
			break;
		case EVENT_RULE_ADD_OR_UPDATE_SUBMIT:
			model.setOpState(addSubmitRule(eventRuleConfigManager, payload.getRuleId(), "",
					payload.getConfigs(), payload.getAvailable()));
			model.setRules(eventRuleConfigManager.getMonitorRules().getRules().values());
			break;
		case EVENT_RULE_DELETE:
			model.setOpState(deleteRule(eventRuleConfigManager, payload.getRuleId()));
			model.setRules(eventRuleConfigManager.getMonitorRules().getRules().values());
			break;
		default:
