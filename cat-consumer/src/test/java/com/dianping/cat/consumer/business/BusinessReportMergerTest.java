...
public class BusinessReportMergerTest {
	...
	public void testBusinessReportMerge() throws Exception {
		String oldXml = new String(getClass().getResourceAsStream("business_base.xml").readAllBytes(), java.nio.charset.StandardCharsets.UTF_8);
		String newXml = new String(getClass().getResourceAsStream("business_merger.xml").readAllBytes(), java.nio.charset.StandardCharsets.UTF_8);
		BusinessReport reportOld = DefaultSaxParser.parse(oldXml);
		BusinessReport reportNew = DefaultSaxParser.parse(newXml);
		String expected = new String(getClass().getResourceAsStream("business_merger_result.xml").readAllBytes(), java.nio.charset.StandardCharsets.UTF_8);
