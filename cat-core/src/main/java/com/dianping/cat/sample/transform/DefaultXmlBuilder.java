...
public class DefaultXmlBuilder implements IVisitor {
   ...
   public void visitDomain(Domain domain) {
      startTag(ENTITY_DOMAIN, true, null, ATTR_ID, domain.getId(), ATTR_SAMPLE, domain.getSample());
   }

   @Override
   public void visitSampleConfig(SampleConfig sampleConfig) {
      startTag(ENTITY_SAMPLE_CONFIG, null);

      if (!sampleConfig.getDomains().isEmpty()) {
         for (Domain domain : sampleConfig.getDomains().values()) {
            domain.accept(m_visitor);
         }
      }

      endTag(ENTITY_SAMPLE_CONFIG);
   }
}
