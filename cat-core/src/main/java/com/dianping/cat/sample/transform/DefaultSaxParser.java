...
import static com.dianping.cat.sample.Constants.ENTITY_SAMPLE_CONFIG;
...
import com.dianping.cat.sample.IEntity;
...
public class DefaultSaxParser extends DefaultHandler {
   ...
   public static SampleConfig parse(InputStream in) throws SAXException, IOException {
      return parseEntity(SampleConfig.class, new InputSource(removeBOM(in)));
   }

   public static SampleConfig parse(Reader reader) throws SAXException, IOException {
      return parseEntity(SampleConfig.class, new InputSource(removeBOM(reader)));
   }

   public static SampleConfig parse(String xml) throws SAXException, IOException {
      return parseEntity(SampleConfig.class, new InputSource(new StringReader(removeBOM(xml))));
   }
   ...
   private void parseForSampleConfig(SampleConfig parentObj, String parentTag, String qName, Attributes attributes) throws SAXException {
      if (ENTITY_DOMAIN.equals(qName)) {
         ...
         throw new SAXException(String.format("Element(%s) is not expected under sample-config!", qName));
      }
   ...
   private void parseRoot(String qName, Attributes attributes) throws SAXException {
      if (ENTITY_SAMPLE_CONFIG.equals(qName)) {
         SampleConfig sampleConfig = m_maker.buildSampleConfig(attributes);

         m_entity = sampleConfig;
         m_objs.push(sampleConfig);
         m_tags.push(qName);
   ...
   public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
      if (uri == null || uri.length() == 0) {
         if (m_objs.isEmpty()) { // root
            ...
            if (parent instanceof SampleConfig) {
               parseForSampleConfig((SampleConfig) parent, tag, qName, attributes);
