...
import static com.dianping.cat.configuration.message.Constants.ELEMENT_START_TYPES;

import static com.dianping.cat.configuration.message.Constants.ENTITY_ATOMIC_MESSAGE_CONFIG;
import static com.dianping.cat.configuration.message.Constants.ENTITY_DOMAIN;
import static com.dianping.cat.configuration.message.Constants.ENTITY_PROPERTY;
import static com.dianping.cat.configuration.message.Constants.ENTITY_PROPERTIES;
...
import com.dianping.cat.configuration.message.IEntity;
...
public class DefaultSaxParser extends DefaultHandler {
   ...
   public static AtomicMessageConfig parse(InputStream in) throws SAXException, IOException {
      return parseEntity(AtomicMessageConfig.class, new InputSource(removeBOM(in)));
   }

   public static AtomicMessageConfig parse(Reader reader) throws SAXException, IOException {
      return parseEntity(AtomicMessageConfig.class, new InputSource(removeBOM(reader)));
   }

   public static AtomicMessageConfig parse(String xml) throws SAXException, IOException {
      return parseEntity(AtomicMessageConfig.class, new InputSource(new StringReader(removeBOM(xml))));
   }
   ...
   public void endElement(String uri, String localName, String qName) throws SAXException {
      if (uri == null || uri.length() == 0) {
         ...
         if (currentObj instanceof Domain) {
            ...
            if (ELEMENT_START_TYPES.equals(currentTag)) {
               domain.setStartTypes(getText());
            } else if (ELEMENT_MATCH_TYPES.equals(currentTag)) {
               domain.setMatchTypes(getText());
            }
         }
      }
   ...
   private void parseForAtomicMessageConfig(AtomicMessageConfig parentObj, String parentTag, String qName, Attributes attributes) throws SAXException {
      if (ENTITY_DOMAIN.equals(qName)) {
         ...
         throw new SAXException(String.format("Element(%s) is not expected under atomic-message-config!", qName));
      }
   ...
   private void parseForDomain(Domain parentObj, String parentTag, String qName, Attributes attributes) throws SAXException {
      if (ELEMENT_START_TYPES.equals(qName) || ELEMENT_MATCH_TYPES.equals(qName) || ENTITY_PROPERTIES.equals(qName)) {
   ...
   private void parseRoot(String qName, Attributes attributes) throws SAXException {
      if (ENTITY_ATOMIC_MESSAGE_CONFIG.equals(qName)) {
         AtomicMessageConfig atomicMessageConfig = m_maker.buildAtomicMessageConfig(attributes);

         m_entity = atomicMessageConfig;
         m_objs.push(atomicMessageConfig);
         m_tags.push(qName);
   ...
   public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
      if (uri == null || uri.length() == 0) {
         if (m_objs.isEmpty()) { // root
            ...
            if (parent instanceof AtomicMessageConfig) {
               parseForAtomicMessageConfig((AtomicMessageConfig) parent, tag, qName, attributes);
